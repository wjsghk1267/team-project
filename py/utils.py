...
def process_video(video_path, lstm_text, yolo_text):
    if not os.path.exists(video_path):
        print(f"오류: 비디오 파일을 찾을 수 없습니다generate_query: {video_path}")
        return None

    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():lstm_keypoint_sequence, skeleton_sequence, breed_counter = process_yolo_results(results)
    ...
    output_path = 'E:/1006/test/output_video_with_labels.mp4'
